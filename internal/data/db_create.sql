...
SELECT * FROM product WHERE lower(tags::text)::text[] && lower(array['Фермерские продукты']::text)::text[] ;
SELECT * FROM product WHERE lower(tags::text)::text[] && lower(?::text)::text[] ;
